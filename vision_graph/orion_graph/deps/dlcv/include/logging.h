...
#ifdef WIN32
#define base_filename(x) strrchr(x,'\\')?strrchr(x,'\\')+1:x
#else
#define base_filename(x) strrchr(x,'/')?strrchr(x,'/')+1:x
#endif
...
#define LOG(severity) \
    LogMessage(base_filename(__FILE__), __LINE__, severity).stream()

#define CHECK(x) \
    if(!(x)) \
        LogMessage(base_filename(__FILE__), __LINE__, FATAL).stream() << "Check failed: " #x << ' '

#define ABORT() \
    LogMessage(base_filename(__FILE__), __LINE__, FATAL).stream() << " Abort " << ' '
