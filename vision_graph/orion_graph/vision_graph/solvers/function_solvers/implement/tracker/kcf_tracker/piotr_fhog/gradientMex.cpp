...
using namespace sse_neon;

#define PI 3.14159265f

// compute x and y gradients for just one column (uses sse)
void grad1_kcf(float *I, float *Gx, float *Gy, int h, int w, int x) {
    int y, y1;
    float *Ip, *In, r;
...
void grad2_kcf(float *I, float *Gx, float *Gy, int h, int w, int d) {
    int o, x, c, a = w * h;
    for (c = 0; c < d; c++)
        for (x = 0; x < w; x++) {
            o = c * a + x * h;
            grad1_kcf(I + o, Gx + o, Gy + o, h, w, x);
        }
}
...
void gradMag_kcf(float *I, float *M, float *O, int h, int w, int d, bool full) {
    int x, y, y1, c, h4, s;
    float *Gx, *Gy, *M2;
    __m128 *_Gx, *_Gy, *_M2, _m;
    float *acost = acosTable_kcf(), acMult = 10000.0f;
    // allocate memory for storing one column of output (padded so h4%4==0)
    h4 = (h % 4 == 0) ? h : h - (h % 4) + 4;
    s = d * h4 * sizeof(float);
    M2 = (float *) alMalloc_kcf(s, 16);
    _M2 = (__m128 *) M2;
    Gx = (float *) alMalloc_kcf(s, 16);
    _Gx = (__m128 *) Gx;
    Gy = (float *) alMalloc_kcf(s, 16);
    _Gy = (__m128 *) Gy;
    // compute gradient magnitude and orientation for each column
    for (x = 0; x < w; x++) {
        // compute gradients (Gx, Gy) with maximum squared magnitude (M2)
        for (c = 0; c < d; c++) {
            grad1_kcf(I + x * h + c * w * h, Gx + c * h4, Gy + c * h4, h, w, x);
            for (y = 0; y < h4 / 4; y++) {
                y1 = h4 / 4 * c + y;
...
void gradQuantize_kcf(float *O, float *M, int *O0, int *O1, float *M0, float *M1,
...
void gradHist_kcf(float *M, float *O, float *H, int h, int w,
    ...
    int *O0, *O1;
    float xb, init;
    O0 = (int *) alMalloc_kcf(h * sizeof(int), 16);
    M0 = (float *) alMalloc_kcf(h * sizeof(float), 16);
    O1 = (int *) alMalloc_kcf(h * sizeof(int), 16);
    M1 = (float *) alMalloc_kcf(h * sizeof(float), 16);
    // main loop
    for (x = 0; x < w0; x++) {
        // compute target orientation bins for entire column - very fast
        gradQuantize_kcf(O + x * h, M + x * h, O0, O1, M0, M1, nb, h0, sInv2, nOrients, full,
                         softBin >= 0);
...
float *hogNormMatrix_kcf(float *H, int nOrients, int hb, int wb, int bin) {
    float *N, *N1, *n;
...
void hogChannels_kcf(float *H, const float *R, const float *N,
...
void hog_kcf(float *M, float *O, float *H, int h, int w, int binSize,
    ...
    gradHist_kcf(M, O, R, h, w, binSize, nOrients, softBin, full);
    // compute block normalization values
    N = hogNormMatrix_kcf(R, nOrients, hb, wb, binSize);
    // perform four normalizations per spatial block
    hogChannels_kcf(H, R, N, hb, wb, nOrients, clip, 0);
    wrFree(N);
    wrFree(R);
}

// compute FHOG features
void fhog_kcf(float *M, float *O, float *H, int h, int w, int binSize,
    ...
    gradHist_kcf(M, O, R1, h, w, binSize, nOrients * 2, softBin, true);
    ...
    // compute block normalization values
    N = hogNormMatrix_kcf(R2, nOrients, hb, wb, binSize);
    // normalized histograms and texture channels
    hogChannels_kcf(H + nbo * 0, R1, N, hb, wb, nOrients * 2, clip, 1);
    hogChannels_kcf(H + nbo * 2, R2, N, hb, wb, nOrients * 1, clip, 1);
    hogChannels_kcf(H + nbo * 3, R1, N, hb, wb, nOrients * 2, clip, 2);
    wrFree(N);
    wrFree(R1);
    wrFree(R2);
}
