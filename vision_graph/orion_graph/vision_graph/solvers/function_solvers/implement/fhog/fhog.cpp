...
void grad1( float *I, float *Gx, float *Gy, int h, int w, int x ) {
  ...
  if( h<4 || h%4>0 || (size_t(I)&15) || (size_t(Gx)&15) ) {
    ...
    _G=(__m128*) Gx; _Ip=(__m128*) Ip; _In=(__m128*) In; _r = sse_neon::SET(r);
    for(y=0; y<h; y+=4) *_G++=sse_neon::MUL(sse_neon::SUB(*_In++,*_Ip++),_r);
  }
  ...
  _r = sse_neon::SET(.5f); _G=(__m128*) Gy;
  for(; y+4<h-1; y+=4, Ip+=4, In+=4, Gy+=4)
    *_G++=sse_neon::MUL(sse_neon::SUB(sse_neon::LDu(*In),sse_neon::LDu(*Ip)),_r);
...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  ...
  for( x=0; x<w; x++ ) {
    ...
    for(c=0; c<d; c++) {
      ...
      for( y=0; y<h4/4; y++ ) {
        y1=h4/4*c+y;
        _M2[y1]=sse_neon::ADD(sse_neon::MUL(_Gx[y1],_Gx[y1]),sse_neon::MUL(_Gy[y1],_Gy[y1]));
        if( c==0 ) continue; _m = sse_neon::CMPGT( _M2[y1], _M2[y] );
        _M2[y] = sse_neon::OR( sse_neon::AND(_m,_M2[y1]), sse_neon::ANDNOT(_m,_M2[y]) );
        _Gx[y] = sse_neon::OR( sse_neon::AND(_m,_Gx[y1]), sse_neon::ANDNOT(_m,_Gx[y]) );
        _Gy[y] = sse_neon::OR( sse_neon::AND(_m,_Gy[y1]), sse_neon::ANDNOT(_m,_Gy[y]) );
      }
    }
    // compute gradient mangitude (M) and normalize Gx
    for( y=0; y<h4/4; y++ ) {
      _m = sse_neon::MIN( sse_neon::RCPSQRT(_M2[y]), sse_neon::SET(1e10f) );
      _M2[y] = sse_neon::RCP(_m);
      if(O) _Gx[y] = sse_neon::MUL( sse_neon::MUL(_Gx[y],_m), sse_neon::SET(acMult) );
      if(O) _Gx[y] = sse_neon::XOR( _Gx[y], sse_neon::AND(_Gy[y], sse_neon::SET(-0.f)) );
    };
    ...
    if( O!=0 && full ) {
      ...
      for( ; y<h-4; y+=4 ) sse_neon::STRu( O[y+x*h],
        sse_neon::ADD( sse_neon::LDu(O[y+x*h]), sse_neon::AND(sse_neon::CMPLT(sse_neon::LDu(Gy[y]),sse_neon::SET(0.f)),sse_neon::SET(PI)) ) );
  ...
  int nb, int n, float norm, int nOrients, bool full, bool interpolate )
{
  ...
  const __m128 _norm=sse_neon::SET(norm), _oMult=sse_neon::SET(oMult), _nbf=sse_neon::SET((float)nb);
  const __m128i _oMax=sse_neon::SET(oMax), _nb=sse_neon::SET(nb);
  ...
  if( interpolate ) for( i=0; i<=n-4; i+=4 ) {
    _o=sse_neon::MUL(sse_neon::LDu(O[i]),_oMult); _o0=sse_neon::CVT(_o); _od=sse_neon::SUB(_o,sse_neon::CVT(_o0));
    _o0=sse_neon::CVT(sse_neon::MUL(sse_neon::CVT(_o0),_nbf)); _o0=sse_neon::AND(sse_neon::CMPGT(_oMax,_o0),_o0); *_O0++=_o0;
    _o1=sse_neon::ADD(_o0,_nb); _o1=sse_neon::AND(sse_neon::CMPGT(_oMax,_o1),_o1); *_O1++=_o1;
    _m=sse_neon::MUL(sse_neon::LDu(M[i]),_norm); *_M1=sse_neon::MUL(_od,_m); *_M0++=sse_neon::SUB(_m,*_M1); _M1++;
  } else for( i=0; i<=n-4; i+=4 ) {
    _o=sse_neon::MUL(sse_neon::LDu(O[i]),_oMult); _o0=sse_neon::CVT(sse_neon::ADD(_o,sse_neon::SET(.5f)));
    _o0=sse_neon::CVT(sse_neon::MUL(sse_neon::CVT(_o0),_nbf)); _o0=sse_neon::AND(sse_neon::CMPGT(_oMax,_o0),_o0); *_O0++=_o0;
    *_M0++=sse_neon::MUL(sse_neon::LDu(M[i]),_norm); *_M1++=sse_neon::SET(0.f); *_O1++=sse_neon::SET(0);
  }
  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
    ...
    if( softBin<0 && softBin%2==0 ) {
      ...
      #define GH(H,ma,mb) H1=H; sse_neon::STRu(*H1,sse_neon::ADD(sse_neon::LDu(*H1),sse_neon::MUL(ma,mb)));
      ...
      if( softBin<0 ) for( ; ; y++ ) {
        yb0 = (int) yb; if(yb0>=hb-1) break; GHinit; _m0=sse_neon::SET(M0[y]);
        if(hasLf) { _m=sse_neon::SET(0,0,ms[1],ms[0]); GH(H0+O0[y],_m,_m0); }
        if(hasRt) { _m=sse_neon::SET(0,0,ms[3],ms[2]); GH(H0+O0[y]+hb,_m,_m0); }
      } else for( ; ; y++ ) {
        yb0 = (int) yb; if(yb0>=hb-1) break; GHinit;
        _m0=sse_neon::SET(M0[y]); _m1=sse_neon::SET(M1[y]);
        if(hasLf) { _m=sse_neon::SET(0,0,ms[1],ms[0]);
          GH(H0+O0[y],_m,_m0); GH(H0+O1[y],_m,_m1); }
        if(hasRt) { _m=sse_neon::SET(0,0,ms[3],ms[2]);
  ...
  int nOrients, int softBin, float clip )
{
  ...
  //wrFreeSafe((void**)&N); wrFreeSafe((void**)&R1); wrFreeSafe((void**)&R2);
}
